...
for CS480: Database Systems with Professor Joe Hummel
...
The purpose of our app was to create a crowdsourced database of all Pokemon currently captured, and then provide players with a mechanism to view the data on those captures mapping to their physical, real-world locations of incidence. Ideally, we would design an API to tap into the Niantic servers and the player's mobile devices, thus feeding data into our app on a realtime basis, but this is obviously impossible. Instead, our data consisted of randomly generated Excel spreadsheets, which were fed into the server, and then accessed through multi-level queries.
